...
public class TC_JOOMLA_ARTICLE_004_Verify_user_can_unpublish_a_published_article extends Configure{
	...
	public void login() {
		...
		LoginPage.getInstance().enterTo.userName(Contant.TC004.userName);
		
		Log.info("Step 3. Enter valid password into Password field ");
		LoginPage.getInstance().enterTo.password(Contant.TC004.passWord);

		Log.info("Step 4. Click on 'Log in' button");
		LoginPage.getInstance().clickTo.buttonLogin();
		
		Log.info("Step 5. Select Content > Article Manager");
		HomePage.getInstance().navigateTo.menuTop(Contant.TC004.articlesPage);
		...
		ArticlesNewPage.getInstance().enterTo.title(Contant.TC004.title);
		
		Log.info("Step 8. Select an item from the 'Category' dropdown list");
		ArticlesNewPage.getInstance().selectItemCategory(Contant.TC004.category);
		
		Log.info("Step 9. Select 'Published' item from 'Status' dropdown list");
		ArticlesNewPage.getInstance().selectItemStatus(Contant.TC004.status);
		
		Log.info("Step 10. Enter value on 'Article Text' text area");
		ArticlesNewPage.getInstance().enterTo.content(Contant.TC004.content);
		...
		verifyTrue(ArticlesPage.getInstance().check.messageArticle(Contant.TC004.messageArticleSaved));
		verifyTrue(ArticlesPage.getInstance().check.resultSearch(Contant.TC004.title));
		
		Log.info("Step 13. Check on the recently added article's checkbox");
		ArticlesPage.getInstance().clickTo.checkBoxTitle(Contant.TC004.title);
		
		Log.info("Step 14. Click on 'Unpublish' icon of the top right toolbar");
		ArticlesPage.getInstance().clickTo.buttonUnpublic();
		
		Log.info("Step 15. Verify the article is unpublished successfullyr");
		verifyTrue(ArticlesPage.getInstance().check.messageArticle(Contant.TC004.messageArticleUnpublished));
		verifyTrue(ArticlesPage.getInstance().check.iconUnpublicArticleInTable(Contant.TC004.title));
