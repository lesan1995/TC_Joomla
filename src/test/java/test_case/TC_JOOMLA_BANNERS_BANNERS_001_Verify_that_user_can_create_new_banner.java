...
public class TC_JOOMLA_BANNERS_BANNERS_001_Verify_that_user_can_create_new_banner extends Configure{
	...
	public void login() {
		...
		LoginPage.getInstance().enterTo.userName(Contant.TCBANNER001.userName);
		LoginPage.getInstance().enterTo.password(Contant.TCBANNER001.passWord);
		...
		HomePage.getInstance().navigateTo.menuTop(Contant.TCBANNER001.clientsPage);
		...
		BannersClientsNewPage.getInstance().enterTo.name(Contant.TCBANNER001.clientsName);
		
		Log.info("Step 7. Enter valid Contact Name to Contact Name textbox");
		BannersClientsNewPage.getInstance().enterTo.contactName(Contant.TCBANNER001.contactName);
		
		Log.info("Step 8. Enter valid Contact Email to Contact Email textbox");
		BannersClientsNewPage.getInstance().enterTo.contactEmail(Contant.TCBANNER001.contactEmail);
		...
		verifyTrue(BannersClientsPage.getInstance().check.messageClient(Contant.TCBANNER001.messageClientSaved));
		verifyTrue(BannersClientsPage.getInstance().check.resultSearch(Contant.TCBANNER001.clientsName));
		
		Log.info("Step 11. Select Components -> Banners -> Categories");
		HomePage.getInstance().navigateTo.menuTop(Contant.TCBANNER001.categoriesPage);
		...
		BannersCategoriesNewPage.getInstance().enterTo.nameCategory(Contant.TCBANNER001.categoryName);
		...
		verifyTrue(BannersCategoriesPage.getInstance().check.messageCategory(Contant.TCBANNER001.messageCategorySaved));
		verifyTrue(BannersCategoriesPage.getInstance().check.resultSearch(Contant.TCBANNER001.categoryName));
		
		Log.info("Step 16. Select Components -> Banners -> Banners");
		HomePage.getInstance().navigateTo.menuTop(Contant.TCBANNER001.bannersPage);
		...
		BannersNewPage.getInstance().enterTo.name(Contant.TCBANNER001.bannerName);
		
		Log.info("Step 19. Select a Category in Category dropdown list");
		BannersNewPage.getInstance().selectItemCategory(Contant.TCBANNER001.categoryName);
		
		Log.info("Step 20. Select a Client in Client dropdown list");
		BannersNewPage.getInstance().selectItemClient(Contant.TCBANNER001.clientsName);
		...
		verifyTrue(BannersPage.getInstance().check.messageBanner(Contant.TCBANNER001.messageBannerSaved));
		verifyTrue(BannersPage.getInstance().check.resultSearch(Contant.TCBANNER001.bannerName,Contant.TCBANNER001.categoryName,Contant.TCBANNER001.clientsName));
	}
	@AfterMethod
